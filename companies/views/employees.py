...
class Employees(Base):
    ...
    def post(self, request):
        ...
        enterprise_id = self.get_enterprise_id(request.user.id)
        signup_user = Authentication.signup(self, name, email, password, 'employee', enterprise_id)
...
class EmployeeDetail(Base):
    ...
    def put(self, request, employee_id):
        ...
        if email != employee.user.email and User.objects.filter(email=email).exists():
            raise APIException('Email já utilizado', code='email_already_use')
    ...
    def delete(self, request, employee_id):
        ...
        if check_if_owner:
            raise APIException('O dono da empresa não pode ser excluído')
